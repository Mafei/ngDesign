...
public class DPNImpl extends EntityImpl implements DPN {
	...
	@Override
	public Endpoint getIncoming(Instance instance, Port port) {
		List<Connection> connections = incomingMap.get(instance).get(port);
		if (connections == null || connections.isEmpty()) {
			return null;
		}
		return connections.get(0).getSourceEndpoint();
	}

	@Override
	public Endpoint getIncoming(Port port) {
		List<Connection> connections = getIncomingConnections(port);
		if (connections == null || connections.isEmpty()) {
			return null;
		}
		return connections.get(0).getSourceEndpoint();
	...
	@Override
	public List<Connection> getIncomingConnections(Port port) {
		return incomingMap.get(getVertex()).get(port);
	}
	...
	public List<Endpoint> getOutgoing(Endpoint endpoint) {
		return newEndpointList(outgoingMap.get(endpoint.getVertex()).get(endpoint.getPort()));
	}
	...
	public List<Connection> getOutgoing(final Port port) {
		return outgoingMap.get(getVertex()).get(port);
	}
	...
	/**
	 * Returns a list of target endpoints from the connections.
	 * 
	 * @param connections
	 *            a list of connections
	 * @return a list of endpoints
	 */
	private List<Endpoint> newEndpointList(List<Connection> connections) {
		if (connections == null) {
		...
		if (newGraph != graph) {
			...
			if (graph != null)
				msgs = ((InternalEObject) graph).eInverseRemove(this, EOPPOSITE_FEATURE_BASE
						- DpnPackage.DPN__GRAPH, null, msgs);
			if (newGraph != null)
	...
	public void setVertex(Vertex newVertex) {
		...
		vertex = newVertex;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, DpnPackage.DPN__VERTEX,
					oldVertex, vertex));
	}
